...
OUT (do not build, even as stubs beyond a header comment, until a milestone admits it — stop and flag instead): auras/buffs/debuffs, stances, dual-wield, movement (units are stationary), stealth, pets, networking, batched multi-env stepping (deferred until a profiled M5 training run shows the ctypes loop is the wall), live-oracle capture, any map geometry beyond a flat plane. RL training code is sanctioned by M5 but ONLY under `python/train/` — never in `sim/`; training experiments must be seeded and reproducible, but the spec/golden-trace rules apply to the env, not to experiment code.
